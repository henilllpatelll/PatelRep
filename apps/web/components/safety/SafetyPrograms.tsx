...
export function SafetyPrograms() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { setError('Unable to load safety programs.') }
  }, [])
  ...
  return (
    <div className="space-y-6">
      {error ? <p className="rounded-lg bg-alert-soft p-2 text-sm text-alert">{error}</p> : null}
