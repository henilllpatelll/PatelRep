...
export function IncidentReview({ redesigned }: { redesigned?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    catch { setError('Unable to load controlled incidents.'); setLoadError(true) }
  ...
  return (
    <section className="rounded-[var(--r-lg)] border border-line bg-surface">
      ...
      {loading ? (
        ...
        <StateBlock
          status={loadError ? 'error' : !incidents.length ? 'empty' : null}
          empty={{ title: t('safety.incidents.empty') }}
          error={{ message: t('safety.incidents.loadError'), onRetry: () => void load() }}
        >
          {list}
        </StateBlock>
      ) : !incidents.length ? (
