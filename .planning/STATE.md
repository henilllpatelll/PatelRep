...
**⚠ Blocking finding for true closure — GM admin + staff safety-info surfaces have NO web UI.** The web `safetyApi` client exposes only 4 staff operations (view training, complete training, view plans, file incident). There is **no web UI** for: GM course management, chemical inventory, drill logging, emergency contacts, the training-compliance dashboard, or the inspector export — and **no staff safety-information (SDS/PPE/procedure) retrieval screen**, which the Phase 3 exit criteria explicitly require in EN+ES. All of this exists as working API endpoints (verified live) but is unreachable from the app. This is a real scope gap, not a bug — Phase 3 web is incomplete against its exit criteria and should not be marked complete until these surfaces are built (or the scope is explicitly renegotiated).
