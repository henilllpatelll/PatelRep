...
progress:
  ...
  completed_plans: 39
  percent: 93
  note: "Phase 6 (PMS & AI expansion) executing — Wave 1 CLOSED 2026-07-28: 06-01 (AI credit accounting + SOP double-log fix) and 06-02 (Opera pilot-flag gate). 06-02 has an open blocker: migration 085 was NOT applied to the live DB (tooling/credential gap in the execution context) — must be applied before 06-05's phase gate. Wave 2 (06-03, 06-04) and Wave 3 (06-05 phase gate) not yet started."
---
...
**Milestone "Hotel Standards Execution Plan": Phases 0–5 all CLOSED + DEPLOYED. Phase 6 (PMS & AI expansion) is executing (2026-07-28) — Wave 1 (06-01, 06-02) CLOSED; Wave 2 (06-03, 06-04) next. 06-02 carries an open blocker (migration 085 not yet applied live — see below).**
...
**OPEN BLOCKER (06-02):** migration `085_opera_pilot_flag.sql` (`ALTER TABLE public.tenants ADD COLUMN opera_pilot_enabled BOOLEAN NOT NULL DEFAULT FALSE`) is written and committed but **NOT applied to the live Supabase DB** (`oacnwalhcpqdabivweki`). The executing sub-agent had no Supabase MCP tool access in its sandboxed toolset, and `supabase db push` was correctly refused as unsafe — the linked project's remote migration history has extensive pre-existing drift (migrations ~035–084 were applied to production via the Supabase MCP tool under auto-generated timestamp version identifiers rather than the matching local numbered filenames; `supabase migration list --linked` shows local 035–084 with a blank "Remote" column and ~50 orphaned timestamp-versioned remote entries). Forcing `db push --include-all` after a `migration repair` would have attempted to re-run all of 035–084 against production — an out-of-scope, high-risk action requiring separate explicit approval, not something to do to unblock a single-column `ALTER TABLE`. No direct Postgres credentials exist anywhere in the local environment (checked all `.env*` files, Windows Credential Manager, supabase CLI temp/config dirs, shell profiles). **Must be resolved (via a session with real Supabase MCP access, or manually via the Supabase Studio SQL Editor) before 06-05's live D-06 GM browser walkthrough** — until then, every Opera endpoint will 403 in production for every hotel (column doesn't exist yet, or once it does, defaults to FALSE and no hotel is enrolled). See `06-02-SUMMARY.md` Escalation section for full detail.

**Next:** Wave 2 — 06-03 (AI copilot RBAC + tenant isolation, depends on 06-01) and 06-04 (Opera webhook signature fix, depends on 06-02). Before 06-05's phase gate: resolve the migration-085 live-apply blocker above.
...
- **Phase 6 blocker (06-02, open):** migration `085_opera_pilot_flag.sql` is committed but NOT applied to the live Supabase DB — the executing sub-agent had no Supabase MCP tool access and CLI `db push` was unsafe due to pre-existing remote migration history drift unrelated to this plan (see 06-02-SUMMARY.md and the 06-02 entry above for full detail). Must be applied (via Supabase MCP or the Studio SQL Editor) before 06-05's live D-06 GM browser walkthrough.
