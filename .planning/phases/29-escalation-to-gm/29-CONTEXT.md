...
- Query shape: `room_readiness_predictions` uses `risk_level = 'HIGH' AND is_acknowledged = FALSE AND escalation_level < 1 AND high_risk_since < now() - interval '60 minutes'`. **Correction (found during phase research, 2026-08-13):** `failure_predictions` has no `risk_level` column — only `risk_score INT` — so its equivalent predicate is `risk_score >= 70` (the same threshold `run_asset_failure_predictions` already uses for its own HIGH-crossing notify trigger), not `risk_level = 'HIGH'`. Scoped per-tenant like every other cron (single cross-tenant query gated by tenant_id in the notify call, matching `check_escalations`'s existing style — no hotel loop).
