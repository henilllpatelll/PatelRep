...
- **D-03:** Opt-out is **provider-level**: rely on Twilio's built-in STOP/START handling. **Mechanism finalized during phase research (2026-07-24):** Twilio's default STOP/START interception does NOT forward to the app's webhook or fire a status callback unless a Messaging Service has Advanced Opt-Out enabled (extra Twilio console config, not just code). Confirmed with the user: use the **reactive** design — catch outbound send-time error code 21610 ("unsubscribed recipient") and set `contact_opted_out_at` at that moment. No custom keyword parsing, no Messaging Service/Advanced Opt-Out provisioning required. Accepted tradeoff: opt-out isn't known until the next send attempt fails (small window before the system learns).
