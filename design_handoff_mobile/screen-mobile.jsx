...
function MobileInspectionQueue() {
  const queue = [
  ...
  const done = [
    { num: '102', cleaner: 'Maria Vega', done: '54m ago', score: 97, status: 'passed' },
    { num: '208', cleaner: 'Tina Aoki', done: '1h ago', score: 78, status: 'reopened', note: 'Bathroom re-check' },
    { num: '101', cleaner: 'Maria Vega', done: '1h ago', score: 94, status: 'passed' },
  ];
  const aiTone = (n) => n >= 90 ? 'ready' : n >= 75 ? 'caution' : 'alert';

  return (
    <Phone height={780} label="Inspection Queue">
      <div style={{ height: 'calc(100% - 44px)', display: 'flex', flexDirection: 'column' }}>
        <MobileHeader big sub="Tue · May 26 · Day shift" title="Inspections — 4 waiting" />
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 100px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Summary */}
          <div style={{
            background: 'var(--surface)', border: '1px solid var(--line)',
            borderRadius: 16, padding: 16, display: 'flex', alignItems: 'center', gap: 18,
          }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span style={{ fontFamily: 'var(--font-display)', fontSize: 30, lineHeight: 1 }}>14</span>
                <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>passed today</span>
              </div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 4 }}>94 avg score · 1 reopened</div>
            </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 4 }}>2 likely pass</div>
            </div>
          </div>

          {/* AI fast-track nudge */}
          <div style={{ background: 'var(--ai-soft)', border: '1px solid var(--ai-line)', borderRadius: 14, padding: '12px 14px' }}>
            <AILabel confidence={92}>Fast-track</AILabel>
            <p style={{ margin: '8px 0 0', fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 15, lineHeight: 1.4 }}>
              <strong style={{ fontStyle: 'normal' }}>103</strong> and <strong style={{ fontStyle: 'normal' }}>107</strong> match the standard cleanly — review photos and pass in one tap.
            </p>
            <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
              <Btn variant="ai" size="sm" icon="check">Review &amp; pass 2</Btn>
              <Btn variant="ghost" size="sm">Not now</Btn>
            </div>
          </div>

          {/* Filter chips */}
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', margin: '-2px 0' }}>
            {[{ l: 'To inspect', n: 4, active: true }, { l: 'Passed', n: 14 }, { l: 'Reopened', n: 1 }, { l: 'VIP', n: 1 }].map((f, i) => (
              <span key={i} style={{
                padding: '5px 11px', borderRadius: 999, whiteSpace: 'nowrap',
                background: f.active ? 'var(--ink)' : 'var(--surface)',
                color: f.active ? 'var(--paper)' : 'var(--ink-2)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {queue.map((q, i) => (
              <div key={i} style={{
                ...
                <RoomNumberTile num={q.num} status="clean" size={48} showDot={false} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 13.5, fontWeight: 500 }}>{q.cleaner}</span>
                    {q.vip && <Pill tone="accent" size="sm" icon="star">VIP</Pill>}
                  </div>
                  <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 3 }}>Done {q.done} · {q.note}</div>
                </div>
          ...
          {/* Done today */}
          <div>
            <SectionLabel hint="3 of 14">Done today</SectionLabel>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {done.map((q, i) => (
                <div key={i} style={{
                  background: 'var(--surface)',
                  border: `1px solid var(--${q.status === 'reopened' ? 'alert-line' : 'line'})`,
                  borderRadius: 12, padding: '11px 13px',
                  display: 'flex', alignItems: 'center', gap: 12,
                  opacity: q.status === 'passed' ? 0.62 : 1,
                }}>
                  <RoomNumberTile num={q.num} status={q.status === 'reopened' ? 'dirty' : 'inspected'} size={44} showDot={false} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{q.cleaner}</div>
                    <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 2 }}>
                      {q.status === 'reopened' ? q.note : `Done ${q.done}`}
                    </div>
                  </div>
                  {q.status === 'passed'
                    ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                        <Mono style={{ fontSize: 12, color: 'var(--ready)', fontWeight: 600 }}>{q.score}</Mono>
                        <Pill tone="ready" size="sm" icon="check">passed</Pill>
                      </span>
                    : <Pill tone="alert" size="sm" icon="alert">reopened</Pill>}
                </div>
              ))}
            </div>
          </div>
...
// ─── Inspector: active in-room inspection ────────────────────────────────────
function MobileInspection() {
  const checklist = [
    { area: 'Bedroom', items: [
      { l: 'Linens crisp & aligned', s: 'ok' },
      { l: 'Pillows angle-set', s: 'ok' },
      { l: 'Nightstand cleared', s: 'ok' },
      { l: 'TV remote sanitized', s: 'ok' },
    ]},
    { area: 'Bathroom', items: [
      { l: 'Mirror streak-free', s: 'flag', why: 'AI flagged smudge · photo 2' },
      { l: 'Tub & grout', s: 'ok' },
      { l: 'Towels rolled', s: 'ok' },
      { l: 'Amenities stocked', s: 'todo' },
    ]},
    { area: 'Floor', items: [
      { l: 'Vacuumed', s: 'ok' },
      { l: 'Baseboards dusted', s: 'ok' },
    ]},
    { area: 'Touch points', items: [
      { l: 'Door handles', s: 'ok' },
      { l: 'Light switches', s: 'ok' },
      { l: 'Thermostat', s: 'ok' },
    ]},
  ];
  const photos = [
  ...
  return (
    <Phone height={780} label="Active Inspection">
      <div style={{ height: 'calc(100% - 44px)', display: 'flex', flexDirection: 'column' }}>
        <MobileHeader
          left={<button style={{ background: 'transparent', border: 'none', padding: 4 }}><Icon name="chevronR" size={18} style={{ transform: 'rotate(180deg)' }} /></button>}
          right={<Mono style={{ fontSize: 11, color: 'var(--ink-3)' }}>1 of 4</Mono>}
          title="Inspect 103"
          sub="Queen · Standard checkout"
        />
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 96px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Hero */}
          <div style={{
            background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16,
            padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14,
          }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 40, lineHeight: 1 }}>103</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <Avatar name="Maria Vega" size={20} />
                <span style={{ fontSize: 13, fontWeight: 500 }}>Maria Vega</span>
              </div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 4 }}>Cleaned in 28m · done 12m ago</div>
            </div>
            <Pill tone="info" size="sm" icon="clock">12m</Pill>
          </div>

          {/* AI pre-check */}
          <div style={{ background: 'var(--ai-soft)', border: '1px solid var(--ai-line)', borderRadius: 14, padding: '14px 16px' }}>
            <AILabel confidence={92}>Pre-check · likely pass</AILabel>
            <p style={{ margin: '10px 0 0', fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 15.5, lineHeight: 1.45 }}>
              Linens crisp, surfaces clean against the standard. One thing to verify — a <strong style={{ fontStyle: 'normal' }}>slight smudge on the mirror</strong> in photo 2.
            </p>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {photos.map((p, i) => (
                <div key={i} style={{
                  aspectRatio: '1', borderRadius: 8, border: `1px solid var(--${p.tone === 'caution' ? 'caution-line' : 'line'})`,
                  background: 'repeating-linear-gradient(135deg, var(--surface-3) 0 8px, var(--surface-2) 8px 16px)',
                  position: 'relative', overflow: 'hidden',
                }}>
                  <div style={{
                    position: 'absolute', bottom: 4, left: 4, right: 4,
                    background: 'rgba(0,0,0,0.62)', color: '#fff', borderRadius: 5,
                    padding: '2px 5px', fontSize: 9, display: 'flex', alignItems: 'center', gap: 4,
                  }}>
                    <StatusDot tone={p.tone} size={5} /> {p.l}
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Checklist */}
          <div>
            <SectionLabel hint="14 items · 1 to verify">Checklist</SectionLabel>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {checklist.map((c, ci) => (
                <div key={ci}>
                  <div style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>{c.area}</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
                    {c.items.map((it, ii) => (
                      <div key={ii} style={{
                        display: 'flex', alignItems: 'center', gap: 11,
                        padding: '12px 13px', minHeight: 48,
                        borderTop: ii > 0 ? '1px solid var(--line-2)' : 'none',
                        background: it.s === 'flag' ? 'var(--caution-soft)' : 'transparent',
                      }}>
                        <span style={{
                          width: 22, height: 22, borderRadius: 6, flexShrink: 0,
                          border: it.s === 'todo' ? '1.5px solid var(--line)' : it.s === 'flag' ? '1.5px solid var(--caution-line)' : 'none',
                          background: it.s === 'ok' ? 'var(--ready)' : 'var(--surface)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                        }}>
                          {it.s === 'ok' && <Icon name="check" size={13} color="#fff" stroke={3} />}
                          {it.s === 'flag' && <Icon name="alert" size={12} color="var(--caution)" stroke={2.2} />}
                        </span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13.5, color: 'var(--ink)', textDecoration: it.s === 'ok' ? 'line-through' : 'none', textDecorationColor: 'var(--ink-4)' }}>{it.l}</div>
                          {it.why && <div style={{ fontSize: 11, color: 'var(--caution)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="spark" size={9} stroke={2.4} /> {it.why}</div>}
                        </div>
                        {it.s === 'flag' && <Pill tone="caution" size="sm">verify</Pill>}
                        {it.s === 'todo' && <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>tap</span>}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            <Btn variant="outline" size="lg" icon="camera" full>Add photo</Btn>
            <Btn variant="outline" size="lg" icon="message" full>Note to Maria</Btn>
          </div>
        </div>

        {/* Sticky decision footer */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '12px 18px 24px', background: 'var(--surface)', borderTop: '1px solid var(--line)', display: 'flex', gap: 8 }}>
          <Btn variant="outline" size="lg" icon="x" style={{ flex: 1 }}>Reopen</Btn>
          <Btn variant="primary" size="lg" icon="check" style={{ flex: 1.4 }}>Pass · Mark ready</Btn>
        </div>
      </div>
    </Phone>
  );
...
// ─── Inspector: reopen / flag issue sheet ────────────────────────────────────
function MobileInspectionReopen() {
  return (
    <Phone height={780} label="Reopen Room">
      <div style={{ height: 'calc(100% - 44px)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        {/* Dimmed context behind sheet */}
        <div style={{ flex: 1, background: 'var(--paper)', padding: '16px 18px', opacity: 0.45, pointerEvents: 'none' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 30, lineHeight: 1 }}>103</div>
          <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 6 }}>Queen · inspecting</div>
        </div>
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(26,24,21,0.32)' }} />

        {/* Bottom sheet */}
        <div style={{
          position: 'absolute', bottom: 0, left: 0, right: 0,
          background: 'var(--surface)', borderRadius: '22px 22px 0 0',
          boxShadow: '0 -12px 40px rgba(26,24,21,0.18)',
          padding: '10px 20px 26px', display: 'flex', flexDirection: 'column', gap: 16,
          maxHeight: '88%',
        }}>
          <div style={{ width: 38, height: 4, borderRadius: 2, background: 'var(--line)', alignSelf: 'center' }} />
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--alert)' }}>Reopen room</div>
            <h1 style={{ margin: '6px 0 0', fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 26, lineHeight: 1.1, letterSpacing: -0.4 }}>
              Send 103 back to Maria
            </h1>
            <p style={{ margin: '8px 0 0', fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.5 }}>
              Pick what didn't pass. She'll get the room back with your notes.
            </p>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { l: 'Mirror streak-free', area: 'Bathroom', on: true },
                { l: 'Amenities stocked', area: 'Bathroom', on: false },
                { l: 'Baseboards dusted', area: 'Floor', on: false },
              ].map((it, i) => (
                <label key={i} style={{
                  display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', minHeight: 48,
                  border: `1px solid var(--${it.on ? 'alert-line' : 'line'})`, borderRadius: 11,
                  background: it.on ? 'var(--alert-soft)' : 'var(--surface)',
                }}>
                  <span style={{
                    width: 22, height: 22, borderRadius: 6, flexShrink: 0,
                    border: it.on ? 'none' : '1.5px solid var(--line)',
                    background: it.on ? 'var(--alert)' : 'var(--surface)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    {it.on && <Icon name="check" size={13} color="#fff" stroke={3} />}
                  </span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13.5, color: 'var(--ink)' }}>{it.l}</div>
                    <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 1 }}>{it.area}</div>
                  </div>
                  <Btn variant="ghost" size="sm" icon="camera">Photo</Btn>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Note */}
          <div>
            <SectionLabel>Note for Maria</SectionLabel>
            <div style={{
              border: '1px solid var(--line)', borderRadius: 11, padding: '11px 13px',
              fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.45, minHeight: 64, background: 'var(--surface-2)',
            }}>
              Mirror has a streak near the bottom corner — quick re-wipe and it's good. Nice work otherwise!
            </div>
            <div style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--ai)' }}>
              <Icon name="spark" size={10} stroke={2.4} /> Drafted from your flag — tap to edit
            </div>
          </div>

          {/* Actions */}
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn variant="ghost" size="lg" style={{ flex: 1 }}>Cancel</Btn>
            <Btn variant="dark" size="lg" icon="arrowR" style={{ flex: 1.5 }}>Reopen &amp; notify</Btn>
          </div>
        </div>
      </div>
    </Phone>
...
Object.assign(window, { Phone, MobileLogin, MobileHome, MobileRoomDetail, MobileMyRooms, MobileCopilot, MobileGuestRequest, MobileWorkOrder, MobileInspectionQueue, MobileInspection, MobileInspectionReopen });
